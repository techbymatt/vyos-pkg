...
class InstallPackagesTests(unittest.TestCase):
    ...
    def run_script(self, *paths):
        return subprocess.run(
            [shutil.which("bash"), str(SCRIPT), *paths],
            check=False,
            env=self.env,
            capture_output=True,
            text=True,
    ...
    def test_coinstallable_packages_use_one_real_batch(self):
        ...
        self.assertEqual(
            self.calls("dpkg-deb"),
            [
                ["--field", self.alpha, "Package"],
                ["--field", self.alpha, "Version"],
                ["--field", self.beta, "Package"],
                ["--field", self.beta, "Version"],
            ],
        )
        self.assertNotIn("WARNING", result.stderr)
        self.assertRegex(
            result.stdout, r"APT simulation: elapsed \d+ seconds \(status 0\)"
        )
        self.assertRegex(
            result.stderr, r"APT batch install: elapsed \d+ seconds \(status 0\)"
        )
    ...
    def test_simulation_conflict_falls_back_in_input_order(self):
        ...
        self.assertIn(
            "WARNING: APT simulation failed; falling back to sequential installs.",
            result.stderr,
        )
        self.assertIn("simulation stdout", result.stderr)
        self.assertIn("simulation diagnostic", result.stderr)
        self.assertRegex(
            result.stderr, r"APT simulation: elapsed \d+ seconds \(status 100\)"
        )
        self.assertRegex(
            result.stderr, r"APT install alpha=1:2.0-3: elapsed \d+ seconds"
        )
    ...
    def test_real_batch_failure_is_not_retried(self):
        ...
        self.assertRegex(
            result.stderr, r"APT batch install: elapsed \d+ seconds \(status 42\)"
        )
        self.assertEqual(list(self.tmp.iterdir()), [])
