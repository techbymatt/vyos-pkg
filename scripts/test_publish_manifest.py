...
class PublishManifestTests(unittest.TestCase):
    ...
    def write_rows(self, rows: list[list[str]]) -> None:
        self.decisions.write_text(''.join('\t'.join(row) + '\n' for row in rows),
                                  encoding='utf-8')

    def create(self) -> pm.Manifest:
        return pm.create_manifest(self.decisions, self.revision, 'd' * 64,
                                  self.image, self.key)
    ...
    def compare(self) -> tuple[int, str, str]:
        return self.cli('compare', '--current', str(self.current),
    ...
    def test_create_canonical_file(self) -> None:
        self.assertEqual(self.cli(
            'create', '--decisions', str(self.decisions),
            '--repository-commit', self.revision, '--patch-commit', 'd' * 64,
            '--image', self.image, '--signing-key', str(self.key),
            '--output', str(self.current)), (0, '', ''))
        manifest = self.create()
        expected = json.dumps(manifest, sort_keys=True, separators=(',', ':'),
                              ensure_ascii=False).encode('utf-8') + b'\n'
        self.assertEqual(self.current.read_bytes(), expected)
        self.assertEqual([pm.package_identity(p) for p in manifest['packages']], [
            ('build', 'node_exporter', 'amd64'),
            ('build', 'node_exporter', 'arm64'), ('build-extra', 'zeta', 'arm64')])
        self.assertEqual(manifest['signing_key_sha256'],
