...
class PublishManifestTests(unittest.TestCase):
    """Manifest creation, comparison, and manifest schema validation."""

    def setUp(self) -> None:
        """Create a temporary workspace with default inputs."""
    ...
    def cli(self, *args: str) -> tuple[int, str, str]:
        """Run main() with captured streams."""
    ...
    def assert_changed(self, changed: pm.Manifest) -> None:
        """Expect a changed published manifest to exit 1."""
    ...
    def assert_malformed(self, text: str) -> None:
        """Expect a malformed published manifest to exit 2."""
        self.current.write_bytes(pm.canonical_bytes(self.create()))
    ...
    def test_create_canonical_file(self) -> None:
        """create writes canonical bytes with sorted packages."""
        self.assertEqual(
            self.cli(
                "create",
                "--packages",
    ...
    def test_compare_ignores_json_format_and_package_order(self) -> None:
        """Formatting and package order alone do not flag changes."""
        manifest = self.create()
    ...
    def test_changed_package_dependencies(self) -> None:
        """Changed package dependencies are flagged."""
        changed = self.create()
        changed["packages"][0]["deps"] = "flex"
        self.assert_changed(changed)

    def test_changed_package_revision(self) -> None:
        """A changed package commit is flagged."""
        changed = self.create()
    ...
    def test_boolean_schema(self) -> None:
        """A non-integer schema_version is rejected."""
    ...
    def test_empty_package_sha(self) -> None:
        """An empty package commit is rejected."""
        self.rows[0]["commit"] = ""
        self.write_rows(self.rows)
    ...
    def test_invalid_utf8_current(self) -> None:
        """Invalid UTF-8 input fails with a diagnostic."""
