...
def validate_manifest(value: object) -> Manifest:
    ...
    fields = {'schema_version', 'repository_commit', 'patch_commit', 'image',
              'signing_key_sha256', 'packages'}
    if not isinstance(value, dict) or set(value) != fields:
        raise ValueError('manifest must contain exactly the schema v1 fields')
    if type(value['schema_version']) is not int or value['schema_version'] != 1:
        raise ValueError('unsupported schema_version')
    repository_commit = require_pattern(value['repository_commit'], REVISION, 'repository_commit')
    ...
    signing_key_sha256 = require_pattern(value['signing_key_sha256'], HEX256, 'signing_key_sha256')
    ...
    for entry in entries:
        if not isinstance(entry, dict) or set(entry) != {'group', 'package', 'arch', 'commit', 'deps'}:
            raise ValueError('package must contain exactly group/package/arch/commit/deps')
        ...
        package = Package(group=group, package=name, arch=arch, commit=commit, deps=deps)
    ...
    return Manifest(
        schema_version=1, repository_commit=repository_commit,
        patch_commit=patch_commit, image=image, signing_key_sha256=signing_key_sha256,
        packages=sorted(packages, key=package_identity))


def create_manifest(decisions: Path, repository_commit: str, patch_commit: str,
                    image: str, signing_key: Path) -> Manifest:
    ...
    with decisions.open(encoding='utf-8', newline='') as stream:
        for number, line in enumerate(stream, 1):
            ...
            if len(fields) != 6:
                raise ValueError(f'decisions line {number}: expected exactly six TSV fields')
            group, outcome, package, arch, commit, deps = fields
            if outcome not in ('hit', 'miss'):
                raise ValueError(f'decisions line {number}: invalid outcome')
            packages.append(Package(group=group, package=package, arch=arch,
                                    commit=commit, deps=deps))
    return validate_manifest(Manifest(
        schema_version=1, repository_commit=repository_commit,
        patch_commit=patch_commit, image=image,
...
def canonical_bytes(manifest: Manifest) -> bytes:
    return json.dumps(validate_manifest(manifest), sort_keys=True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == 'create':
            manifest = create_manifest(args.decisions, args.repository_commit,
                                       args.patch_commit, args.image, args.signing_key)
